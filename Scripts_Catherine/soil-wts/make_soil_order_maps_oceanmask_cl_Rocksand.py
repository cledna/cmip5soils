...
for i in range(1,3):
    order_map = (fin.variables['order'][:] == (i-3)) * 1.
    ...
    fname_out = soil_order_dict[(i-3)]+'_derezed10_cl.nc'
    fout = nc4.Dataset(fname_out, 'w')
    fout.createDimension('lat', JM_o)
    fout.createDimension('lon', IM_o)    
    outVar = fout.createVariable(soil_order_dict[(i-3)], 'f4', fin.variables['order'].dimensions)
