## Process output from CMIP5 tsl analysis ##
## Created by Claire Phillips July 11, 2014 ##

## NOTES ##
#This should skip mrlsl loops if there is no model output
...
# 6) Map of 2080-2100 average (AnomGlobalMap)
...
MODEL=c("bcc-csm1-1","BNU-ESM","CanESM2","CCSM4","CESM1-BGC","MPI-ESM-LR", "GFDL-ESM2G","GISS-E2-R","HadGEM2-ES","inmcm4","IPSL-CM5A-LR","MIROC5","MRI-CGCM3","NorESM1-M") #,"CCSM4","CNRM-CM5", )
XP=c("rcp45","rcp85")
REAL="r1i1p1"
XPSTART=200601
XPEND=210012
...
for(d in 1:length(MODEL)){
  ...
  File=paste(WD,"/tsl_",MODEL[d],"_198601-200512_yearmin_GreatPlains.nc",sep="")
  Tsl.nc<-open.nc(File)
  if(MODEL[d]=="CNRM-CM5") Z=c(0.01, 1.5, 2.1) else Z=var.get.nc(Tsl.nc,"depth")
  if(MODEL[d]=="CNRM-CM5") NZ=3 else NZ=dim.inq.nc(Tsl.nc,"depth")$length
  LZ=var.get.nc(Tsl.nc,"depth_bnds")
  close.nc(Tsl.nc)

  GreatPlainsProfile=data.frame(Depth=c(200,Z*-100),Min=rep(0,NZ+1),Mean=rep(0,NZ+1),Max=rep(0,NZ+1))
  Level=c("yearmin","yearmean","yearmax")
  for(s in 1:3){
    File=paste(WD,"/tas_",MODEL[d],"_198601-200512_",Level[s],"_GreatPlains.nc",sep="")
    Tas.nc<-open.nc(File)
    Tas<-var.get.nc(Tas.nc,"tas",start=c(1,1,1),count=c(1,1,1))
    close.nc(Tas.nc)
    File=paste(WD,"/tsl_",MODEL[d],"_198601-200512_",Level[s],"_GreatPlains.nc",sep="")
    Tsl.nc<-open.nc(File)
    if(MODEL[d]=="CNRM-CM5") NZ=3 else NZ=dim.inq.nc(Tsl.nc,"depth")$length
    #print.nc(Tsl.nc)
    Tsl<-var.get.nc(Tsl.nc,"tsl",start=c(1,1,1,1),count=c(1,1,NZ,1))
    close.nc(Tsl.nc)
    GreatPlainsProfile[,(s+1)]<-c(Tas,Tsl)
  }
  GreatPlainsProfile[,2:4]<-GreatPlainsProfile[,2:4]-273
...
#Plot mesonet data
{Mesonet<-read.csv("F:/Observations/Mesonet/MesonetProfilesMonthly.csv")
 Depths<-c(200,-5,-10,-30)
 plot(Depths~unlist(Mesonet[137,c("TA.Mean","TS.5sod.Mean","TS.10sod.Mean","TS.30sod.Mean")]),type="o",xlim=c(-20,35),ylim=c(-500,300),pch=16,cex=1.2)
 arrows(unlist(Mesonet[137,c("TA.Mean","TS.5sod.Mean","TS.10sod.Mean","TS.30sod.Mean")]),Depths,unlist(Mesonet[137,c("TA.Mean","TS.5sod.Mean","TS.10sod.Mean","TS.30sod.Mean")])+unlist(Mesonet[138,c("TA.Mean","TS.5sod.Mean","TS.10sod.Mean","TS.30sod.Mean")]),Depths,length=0,angle=90)
 arrows(unlist(Mesonet[137,c("TA.Mean","TS.5sod.Mean","TS.10sod.Mean","TS.30sod.Mean")]),Depths,unlist(Mesonet[137,c("TA.Mean","TS.5sod.Mean","TS.10sod.Mean","TS.30sod.Mean")])-unlist(Mesonet[138,c("TA.Mean","TS.5sod.Mean","TS.10sod.Mean","TS.30sod.Mean")]),Depths,length=0,angle=90)
 lines(Mesonet[137,c("TA.Min","TS.5sod.Min","TS.10sod.Min","TS.30sod.Min")],Depths,type="o",col="blue",pch=17,cex=1.2)
 arrows(unlist(Mesonet[137,c("TA.Min","TS.5sod.Min","TS.10sod.Min","TS.30sod.Min")]),Depths,unlist(Mesonet[137,c("TA.Min","TS.5sod.Min","TS.10sod.Min","TS.30sod.Min")])+unlist(Mesonet[138,c("TA.Min","TS.5sod.Min","TS.10sod.Min","TS.30sod.Min")]),Depths,length=0,angle=90, col="blue")
 arrows(unlist(Mesonet[137,c("TA.Min","TS.5sod.Min","TS.10sod.Min","TS.30sod.Min")]),Depths,unlist(Mesonet[137,c("TA.Min","TS.5sod.Min","TS.10sod.Min","TS.30sod.Min")])-unlist(Mesonet[138,c("TA.Min","TS.5sod.Min","TS.10sod.Min","TS.30sod.Min")]),Depths,length=0,angle=90, col="blue")
 lines(Mesonet[137,c("TA.Max","TS.5sod.Max","TS.10sod.Max","TS.30sod.Max")],Depths,type="o",col="red",pch=15,cex=1.2)
 arrows(unlist(Mesonet[137,c("TA.Max","TS.5sod.Max","TS.10sod.Max","TS.30sod.Max")]),Depths,unlist(Mesonet[137,c("TA.Max","TS.5sod.Max","TS.10sod.Max","TS.30sod.Max")])+unlist(Mesonet[138,c("TA.Max","TS.5sod.Max","TS.10sod.Max","TS.30sod.Max")]),Depths,length=0,angle=90, col="red")
 arrows(unlist(Mesonet[137,c("TA.Max","TS.5sod.Max","TS.10sod.Max","TS.30sod.Max")]),Depths,unlist(Mesonet[137,c("TA.Max","TS.5sod.Max","TS.10sod.Max","TS.30sod.Max")])-unlist(Mesonet[138,c("TA.Max","TS.5sod.Max","TS.10sod.Max","TS.30sod.Max")]),Depths,length=0,angle=90, col="red")
 abline(h=0,col="grey")
...
var.get.nc(AnomGlobalts.nc,"time")
  ...
  File=paste(WD,"/",VAR,"_",MODEL[d],"_",XP[x],"_GlobalMean_2080-2100.nc",sep="")
  ...
  ## 2080-2100 MAP DELTA
  File=paste(WD,"/",VAR,"_",MODEL[d],"_",XP[x],"_Map_2080-2100.nc",sep="")
  ...
  ## 2080-2100 MAP TAS AMPLITUDE
  File=paste(WD,"/",VAR,"_",MODEL[d],"_",XP[x],"_208001-210012_land_NoIce_amplitude.nc",sep="")
  ...
  for (s in 1:length(SOIL)){
    ...
    File=paste(WD,"/",VAR,"_",MODEL[d],"_",XP[x],"_GlobalMean_2080-2100_",SOIL[s],".nc",sep="")
  ...
  #Global zonal mean
  File=paste(WD,"/",VAR,"_",MODEL[d],"_",XP[x],"_latmean_2080-2100_Global.nc",sep="")
  ...
  for (k in 1:length(LATZONE)){
    File=paste(WD,"/",VAR,"_",MODEL[d],"_",XP[x],"_latmean_2080-2100_",LATZONE[k],".nc",sep="")
  ...
  ZoneMonthly.arr<-array(data=NA,dim=c(length(ZONE),12,2),dimnames=list(ZONE,c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"),c("1986-2005","2080-2100")))
  ...
  for (j in 1: length(ZONE)){
    ...
    #EOC monthly temp
    File=paste(WD,"/",VAR,"_",MODEL[d],"_",XP[x],"_2080-2100_mon_",ZONE[j],".nc",sep="")
    ...
    #2080-2100 anomaly mean
    File=paste(WD,"/",VAR,"_",MODEL[d],"_",XP[x],"_2080-2100_anom_region_",ZONE[j],".nc",sep="")
...
for(d in 7:length(MODEL)){
  x=2#for(x in 1:2){ #loop through XP[x]s
  for (m in 1:2){
  #if(MODEL[d]=="CCSM4") REAL<-"r2i1p1" else REAL<-"r1i1p1"
  WD=paste("F:/wd",MODEL[d],sep="/")
  REALM="Lmon"
  ...
  File=paste(WD,"/",VAR[m],"_",MODEL[d],"_",XP[x],"_GlobalMean_2080-2100.nc",sep="")
  AnomGlobalmean.nc<-open.nc(File)
 # print.nc(AnomGlobalmean.nc)
  ...
  ## 2080-2100 MAP DELTA
  File=paste(WD,"/",VAR[m],"_",MODEL[d],"_",XP[x],"_Map_2080-2100.nc",sep="")
  AnomGlobalmap.nc<-open.nc(File)
  #print.nc(AnomGlobalmap.nc)
  NLON=dim.inq.nc(AnomGlobalmap.nc,"lon")$length
  NLAT=dim.inq.nc(AnomGlobalmap.nc,"lat")$length
 ...
 ## Global mean for soil orders
  for (s in 1:length(SOIL)){
    File=paste(WD,"/",VAR[m],"_",MODEL[d],"_",XP[x],"_GlobalMean_2080-2100_",SOIL[s],".nc",sep="")
    AnomSoilGlobalMean.nc<-open.nc(File)
...
#Global zonal mean (interpolate to 0.01,0.1, and 1.0m depth)
  File=paste(WD,"/",VAR[m],"_",MODEL[d],"_",XP[x],"_latmean_2080-2100_Global.nc",sep="")
  GlobalLatMean.nc<-open.nc(File)
...
for (k in 1:length(LATZONE)){
  File=paste(WD,"/",VAR[m],"_",MODEL[d],"_",XP[x],"_latmean_2080-2100_",LATZONE[k],".nc",sep="")
  SoilLatMean.nc<-open.nc(File)
...
#Historic monthly means
ZoneMonthly.arr<-array(data=NA,dim=c(length(ZONE),NZ+3,12,2),dimnames=list(ZONE,paste(c(round(MDPNT,3),0.01,0.1,1.0),"m",sep=""),month.abb,c("1986-2005","2080-2100")))
...
for (j in 1: length(ZONE)){
  ...
  File=paste(WD,"/",VAR[m],"_",MODEL[d],"_",XP[x],"_2080-2100_mon_",ZONE[j],".nc",sep="")
  ZoneEOCMon.nc<-open.nc(File)
  ...
  #2080-2100 anomaly mean
  File=paste(WD,"/",VAR[m],"_",MODEL[d],"_",XP[x],"_2080-2100_anom_region_",ZONE[j],".nc",sep="")
  ZoneAnomTimean.nc<-open.nc(File)
  #print.nc(ZoneAnomTimean.nc)
...
File=paste(WD,"/",VAR[m],"_",MODEL[d],"_",XP[x],"_208001-210012_land_NoIce_amplitude.nc",sep="")
EOCAmplitudemap.nc<-open.nc(File)
...
#### Plots ####
## Plot damping depth
# load("F:/wd/bcc-csm1-1/bcc-csm1-1_rcp45_r1i1p1_tsl.RData")
# load("F:/wd/bcc-csm1-1/bcc-csm1-1_rcp45_r1i1p1_tas.RData")
# load("F:/wd/bcc-csm1-1/bcc-csm1-1_rcp45_r1i1p1_mrlsl.RData")
# ls()
# Damp<-get(paste(MODEL,REAL,"DampingDepth",sep="."))[,,1]
# SoilOrders<-get(paste(MODEL,REAL,"DampingDepth",sep="."))[,,2]
# tas.hist<-get(paste(MODEL,REAL,"tas.HistAbsolutemap",sep="."))
# tsl.hist<-get(paste(MODEL,REAL,"tsl.HistAbsolutemap",sep="."))[,,1]
# tsl.amp<-get(paste(MODEL,REAL,"tsl.HistAmplitudemap",sep="."))[,,1]
# mrlsl.hist<-get(paste(MODEL,REAL,"mrlsl.HistAbsolutemap",sep="."))[,,1]
# image.plot(Damp)
# image.plot(tsl.amp)
# image.plot(tas.hist)
# image.plot(mrlsl.hist)
# image.plot(tsl.hist-tas.hist, main="bcc-csm1-1 soil minus air temp", zlim=c(-5,13))
# tas.EOC<-get("bcc-csm1-1.rcp45.r1i1p1.tas.AnomGlobalmap")
# tsl.EOC<-get("bcc-csm1-1.rcp45.r1i1p1.tsl.AnomGlobalmap")[,,1]
# image.plot(tsl.EOC-tas.EOC, main="bcc-csm1-1 soil warming minus air warming", zlim=c(-5,13))
# 
# Compile<-data.frame(Damp=c(Damp),mrlsl=c(mrlsl.hist),temp.amp=c(tsl.amp),tas.mean=c(tas.hist),SoilOrder=c(SoilOrders))
# names(Compile)
# plot(na.omit(Compile)) #Plot x-y scatter plots of all variables
# #Plot damping depth by soil order
# plot(Damp~SoilOrder,data=Compile,xaxt="n",ylab="Damp depth (z)") 
# axis(side=1,labels=FALSE)
# text(seq(2,by=1,length.out=14),par("usr")[3]-0.25,srt=45,adj=1,labels=SOIL, xpd=TRUE)
# 
# #Plot a few soil orders with different texture for damp ~ moisture and damp ~ temp.amp
# plot(Damp~temp.amp,data=Compile[Compile$SoilOrder==11,],col="green",pch=16,xlim=c(0,35),ylim=c(0,6),ylab="Damp depth",xlab="Annual Temp Amplitude")
# points(Damp~temp.amp,data=Compile[Compile$SoilOrder==5|Compile$SoilOrder==6|Compile$SoilOrder==8,],col="orange",pch=16)
# points(Damp~temp.amp,data=Compile[Compile$SoilOrder==4,],col="dark blue",pch=16)
# points(Damp~temp.amp,data=Compile[Compile$SoilOrder==13,],col="grey",pch=16)
# legend("topleft",legend=c("Mollisol","Oxisol,Ultisol,Vertisol","Histosol","Inceptisol"),pch=16,col=c("green","orange","dark blue","grey"))
# 
# plot(Damp~mrlsl.hist,data=Compile[Compile$SoilOrder==11,],col="green",pch=16,xlim=c(0,35),ylim=c(0,6),ylab="Damp depth",xlab="Annual Temp Amplitude")
# points(Damp~mrlsl.hist,data=Compile[Compile$SoilOrder==5|Compile$SoilOrder==6|Compile$SoilOrder==8,],col="orange",pch=16)
# points(Damp~mrlsl.hist,data=Compile[Compile$SoilOrder==4,],col="dark blue",pch=16)
# points(Damp~mrlsl.hist,data=Compile[Compile$SoilOrder==13,],col="grey",pch=16)
# legend("topleft",legend=c("Mollisol","Oxisol,Ultisol,Vertisol","Histosol","Inceptisol"),pch=16,col=c("green","orange","dark blue","grey"))
# 
# Color<-grey(na.omit(c(tsl.amp))/36)
# par(mar=c(5,5,2,2))
# plot(na.omit(c(Damp))~na.omit(c(mrlsl.hist)),col=Color,pch=16, xlab="Soil moisture",ylab="Damping depth")
# plot(na.omit(c(Damp))~na.omit(c(mrlsl.hist)),col=na.omit(c(SoilOrders)),pch=16, xlab="Soil moisture",ylab="Damping depth")

# apply(AnomTs.arr[,1,76:95],MARGIN=1,mean, na.rm=TRUE)

# Make sure tsl is loaded
# MODEL="bcc-csm1-1"
# XP[x]="rcp85"
# WD="E:/wd/RData"
# load(paste(WD,"/",MODEL,"_",XP[x],"_",REAL,"_","tsl.RData",sep=""))
# 
# Annualts.temp<-get(paste(MODEL,XP[x],REAL,"tsl.Annualts.arr",sep="."))
# dev.off()
# plot(2006:2100,Annualts.temp[1,"0.01m",],type="l", ylab=eXP[x]ression(symbol(D)~Temp),xlab="Year",ylim=c(-4,8))
# for(i in 1:length(ZONE)){
#   lines(2006:2100,Annualts.temp[(i+15),"0.01m",],type="l",col=(i+1))
# }
# legend(x="topleft",legend=c("Global",ZONE),col=1:5,lty=1)
# 
# 
# Zone.temp<-get(paste(MODEL,XP[x],REAL,"tsl.ZoneMonthly.arr",sep="."))
# par(mfrow=c(2,2),mar=c(4,4,1,1),tck=0.02,mgp=c(2,0.5,0))
# for (i in 1:length(ZONE)){
#   plot(x=1:12,y=Zone.temp[i,"0.01m",,1], col=i, lty=1,ylab="Temp",xlab="Month",type="o",ylim=c(230,330),xaxt="n")
#   abline(h=276.15)
#   axis(side=1,at=1:12,labels=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"),tck=0.02,mgp=c(2,0.5,0))
#   lines(x=1:12,y=Zone.temp[i,"0.01m",,2], col=i, lty=2)
#   legend(x="topleft",legend=c("1986-2005","2080-2100"),col=i,lty=c(1,2))
#   text(10,320,ZONE[i])
# }
# 
# LatZone.temp<-get(paste(MODEL,XP[x],REAL,"tsl.LatZone.arr",sep="."))
# dev.off()
#   plot(seq(-90,90,length.out=NLAT),LatZone.temp[1,,"0.01m"], type="l", ylab=eXP[x]ression(symbol(D)~Temp),xlab="Latitude",tck=0.03, mgp=c(2,0.5,0), ylim=c(-1,9))
#   for (i in 1:length(LATZONE)){
#     lines(seq(-90,90,length.out=NLAT),LatZone.temp[(i+1),,"0.01m"], col=(i+1), lty=1)
#   }
#   legend(x="topleft",legend=c("Global",LATZONE),col=1:(length(LATZONE)+1),lty=1:(length(LATZONE)+1))
# 
# LatZone.interp.temp<-LatZone.interp.arr
# plot(seq(-90,90,length.out=180),LatZone.interp.temp[1,,"0.01m"], type="l", ylab=eXP[x]ression(symbol(D)~Temp),xlab="Latitude",tck=0.03, mgp=c(2,0.5,0), ylim=c(-1,9),col=2)
# for (i in 1:length(LATZONE)){
#   lines(seq(-90,90,length.out=180),LatZone.interp.temp[(i+1),,"0.01m"], col=(i+2), lty=1)
# }
# legend(x="topleft",legend=c("Global",LATZONE),col=1:(length(LATZONE)+1),lty=1:(length(LATZONE)+1))
# 
